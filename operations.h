...
/**
 * @brief this functions handles all the math and logical operations between two
 * operands
 *
 * @param operand1 the node of the first operand
 * @param operand2 the node of the second operand
 * @param operator_type the type of the operation whether ADD, SUB, MUL, DIV,
 * MOD, OR, AND, EQUAL_TO, NOT_EQUAL, GT, GTE, LT, LTE
 * @return struct conNodeType* the node of the result of the operation
 */
struct conNodeType* handle_operations(struct conNodeType* operand1,
  ...
  if ((operand1->type != typeInt && operand1->type != typeFloat &&
       operand1->type != typeBool) ||
      (operand2->type != typeInt && operand2->type != typeFloat &&
       operand2->type != typeBool)) {
    yyerror(
        "ERROR: can't make math/logical operations to non-int, non-float "
        "operands\n");
    return NULL;
  }
  ...
  float opr1_value =
      operand1->type == typeFloat ? operand1->fValue : (int)operand1->iValue;

  float opr2_value =
      operand2->type == typeFloat ? operand2->fValue : (int)operand2->iValue;
  ...
  switch (operator_type) {
    ...
    case OR:
      // TODO: check if we need this check or we can allow logical
      // operations on int
      //  if(operand1->type != typeBool || operand2->type != typeBool) {
      //    yyerror("ERROR: can't make logical operations to non-bool
      //    operands\n"); return NULL;
      //  }
 ...
 */
struct conNodeType* handle_unary_operations(struct conNodeType* operand1,
                                            int operator_type) {
  if (operand1 == NULL) {
    yyerror("ERROR: NULL operand!\n");
    return NULL;
