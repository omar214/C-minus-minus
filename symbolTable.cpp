 ...
 * @param scope_dir integer value to indicates whether we will go up or down in the scope
 */
 ...
 */
struct conNodeType* insertNewVariable(
  char* var_name,
  conEnum var_type,
  ...
  while(table_iterator != NULL){
    bool is_found = table_iterator->symtable.find(var_name) != table_iterator->symtable.end();
    if(is_found){
      ...
      bool is_type_missmatch = table_iterator->symtable[var_name].first.type != var_value.type;
      ...
      if(is_redeclared){
        *error = (char *)malloc(sizeof(char) * 100);
        strcpy(*error, "Error: re-declaration of the variable in the same scope");
        return NULL;
      }
      
      // update the value of the variable
      table_iterator->symtable[var_name].first = var_value; // the value of the variable
      table_iterator->symtable[var_name].second = make_pair(false, true); // is_const= false, has_value = true
  ...
  if(is_not_declared){
    *error = (char *)malloc(sizeof(char) * 100);
    strcpy(*error, "Error: variable is not declared in the current scope , can't update it");
    return NULL;
  }


  bool is_type_missmatch = (has_value && var_type != var_value.type && var_type != typeVoid);
  if(is_type_missmatch){
    *error = (char *)malloc(sizeof(char) * 100);
    strcpy(*error, "Error: type missmatch for the variable");
    return NULL;
...
void printSymbolTable(){
  ...
  symbol_table_file << "------------------ Start SYMBOL TABLE ------------------" << endl;
  ...
  for (auto &it: table_iterator->symtable) {
    ...
    if(var_name == "main"){
      // since function are saved in the symbol table also && we need to neglect the main
      continue;
    }

    cout << "variable_name:\t" << var_name 
         << " | type:\t" << var_type_str 
  ...
  symbol_table_file << "-------------------End Symbol Table-----------------" << endl;
